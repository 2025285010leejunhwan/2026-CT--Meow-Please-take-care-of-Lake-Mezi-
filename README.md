...
```text
📦 202X-CT-FINAL-PROJECT
 ┣ 📂 bgm                # 게임 배경 음악 (MP3)
 ┣ 📂 sfx                # 효과음 (캐스팅, 릴링, 발소리 등)
 ┣ 📂 곤지               # NPC 곤지 애니메이션 스프라이트
 ┣ 📂 맵                 # 스테이지별 배경 및 로비 이미지
 ┣ 📂 물고기             # 일반/변이 물고기 및 보스 이미지
 ┣ 📂 플레이어           # 플레이어 이동(상하좌우) 스프라이트
 ┣ 📜 final project.py   # 메인 게임 실행 코드 (Source Code)
 ┣ 🖼️ 게임 로고.png      # 타이틀 로고
 ┣ 🖼️ 낚싯대 level1.jpg  # 아이템 이미지 (기본)
 ┣ 🖼️ 낚싯대 level2.jpg  # 아이템 이미지 (업그레이드)
 ┣ 🖼️ 박사님.png         # NPC 박사님 이미지
 ┗ 🖼️ 시작화면.jpg       # 게임 시작 화면 이미지
